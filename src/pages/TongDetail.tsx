// 통 HADA - 통 상세 (5개 탭: 기본 정보 / 입력 / AI 요약 / 후속 과제 / 조직 데이터)
...
import { ActionItemsTab } from '@/pages/tong/ActionItemsTab'
import { OrgDataTab } from '@/pages/tong/OrgDataTab'

const TABS = ['기본 정보', '입력', 'AI 요약', '후속 과제', '조직 데이터'] as const
...
export function TongDetail() {
  ...
  async function handleDelete() {
    if (!tong) return
    if (!confirm(`'${tong.title}' 통을 삭제하시겠습니까? 관련 기록과 후속 과제도 함께 삭제됩니다.`)) return
  ...
  return (
    <div>
      ...
      {tab === '후속 과제' && <ActionItemsTab tong={tong} />}
