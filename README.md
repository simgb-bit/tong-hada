...
> 폴더 내비게이션은 **좌측 전역 사이드바**(통 기록함 메뉴 하위)에 있으며, 선택은 URL 쿼리(`/tongs?f=<all|mine|shared|trash|폴더id>`)로 관리해 사이드바와 기록함 페이지가 공유합니다. 개인 폴더는 **하위 폴더(무제한 깊이)** 를 지원합니다([`src/components/FolderNav.tsx`](src/components/FolderNav.tsx)).
...
- **개인 폴더** (사용자 생성, 다중 분류, **하위 폴더 지원**)
  - 만들기(루트/하위) / 이름 변경 / 삭제(하위 폴더까지 재귀 정리). 트리 펼침·접힘. 부모 폴더는 **직접 담긴 통만** 표시
